...
from key_manager.typings import ExitSignature, ValidatorKeystore
...
def validators_exit(vault: HexAddress, consensus_endpoint: str) -> None:
    ...
    for keystore in keystores_and_passwords:
        exit_signature = get_exit_signature(
                            keystore.index,
                            HexStr(keystore.private_key),
                            fork,
                            config.network
                        )
...
def _get_consensus_client(consensus_endpoint: str) -> Beacon:
    try:
        ...
        is_syncing = consensus_client.get_syncing()['data']['is_syncing']
        if is_syncing:
...
def get_exit_signature(
    ...
    return ExitSignature(w3.to_hex(exit_signature))
